...
def arcsinh_transform(
    ...
    scale_factor: np.integer | np.floating = 150,
    ...
    for table in table_keys:
        ...
        if sparse.issparse(adata.X):
            transformed_X: NDArray = sparse.csr_matrix(np.arcsinh(shift_factor + (adata.X.toarray() * scale_factor)))
        else:
            transformed_X: NDArray = sparse.csr_matrix(np.arcsinh(shift_factor + (adata.X * scale_factor)))
